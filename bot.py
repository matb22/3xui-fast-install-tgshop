...
# Инициализация API панели 3x-ui
xui = XuiAPI(url=XUI_URL)

# База данных временных платежей ЮMoney
pending_payments = {}
...
async def grant_subscription(user_id: int, username: str, days: int) -> tuple[str, str, int]:
    ...
    if user_record:
        client_uuid, client_email, current_expiry = user_record
        now_ms = int(time.time() * 1000)
        start_ms = max(current_expiry, now_ms)
        new_expiry = start_ms + added_ms
    else:
        client_uuid = str(uuid.uuid4())
        client_email = f"tg_{user_id}_{username or 'user'}"
...
def get_tariffs_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="🔥 1 месяц — 75₽", callback_data="buy_1m")
    builder.button(text="⚡️ 3 месяца — 175₽", callback_data="buy_3m")
...
async def back_to_main(callback: types.CallbackQuery):
    # Если прошлое сообщение содержало фото (например, кабинет или инструкции),
    # лучше удалить его и отправить новое чистое меню, чтобы не ломать разметку.
...
async def show_tariffs_inline(callback: types.CallbackQuery):
    text = (
        "⚡️ **Выберите тарифный план:**\n\n"
        "Все тарифы включают высокую скорость, поддержку Discord (режим TUN) и безлимитный трафик."
    )
    if callback.message.photo:
        await callback.message.delete()
        await callback.message.answer(text, reply_markup=get_tariffs_keyboard(), parse_mode="Markdown")
    else:
        await callback.message.edit_text(text, reply_markup=get_tariffs_keyboard(), parse_mode="Markdown")
    await callback.answer()

# НАЖАТИЕ: ВЫБОР ТАРИФА
...
async def show_cabinet_inline(callback: types.CallbackQuery):
    ...
    if not user_record:
        ...
        # Собираем клавиатуру с возможностью купить или вернуться
        builder = InlineKeyboardBuilder()
        builder.button(text="🛍 Купить VPN", callback_data="menu_buy")
        builder.button(text="⬅️ Назад", callback_data="to_main")
        builder.adjust(1)
    ...
    client_uuid, client_email, expiry_time_ms = user_record
    now_ms = int(time.time() * 1000)
    ...
    # Чтобы не плодить сообщения, удаляем старое текстовое меню и присылаем карточку с QR-кодом
    await callback.message.delete()
